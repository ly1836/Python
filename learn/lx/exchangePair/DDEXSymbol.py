...
class dbo:
    ...
    def DDEX(self, url, exchangePair, exchangeName, exchangeId, exchangeUrl):
        counts = 0
        headers = {
        ...
        try:
            ...
            if respone.text.__len__() > 1:
                d = json.loads(respone.text)['data']['markets']
                for i in range(0, len(d)):
                    # 完整交易对
                    symbol = d[i]['id']

                    # 基础币种
                    baseCurrency = str(symbol).split("-")[0]
                    # 计价币种
                    quoteCurrency = str(symbol).split("-")[1]
...
if __name__ == "__main__":
    ...
    db.DDEX("https://api.ddex.io/v2/markets", exchangePair,"DDEX",162,"https://ddex.io/")
    print("DDEX交易对同步完毕")
